...
if [[ "${NODE_OS_DISTRIBUTION}" == "gci" || "${NODE_OS_DISTRIBUTION}" == "ubuntu" || "${NODE_OS_DISTRIBUTION}" == "centos" ]]; then
...
function set-node-image() {
    ...
    elif [[ "${NODE_OS_DISTRIBUTION}" == "centos" ]]; then
        NODE_IMAGE=filbranden-centos-gke-3
        NODE_IMAGE_PROJECT=filbranden-cloud
    fi
}
...
EOF
  ...
  if [[ "${master}" == "true" && ("${MASTER_OS_DISTRIBUTION}" == "trusty" || "${MASTER_OS_DISTRIBUTION}" == "gci") || "${MASTER_OS_DISTRIBUTION}" == "ubuntu" ]] || \
     [[ "${master}" == "false" && ("${NODE_OS_DISTRIBUTION}" == "trusty" || "${NODE_OS_DISTRIBUTION}" == "gci") || "${NODE_OS_DISTRIBUTION}" = "ubuntu" || "${NODE_OS_DISTRIBUTION}" == "centos" ]] ; then
    cat >>$file <<EOF
